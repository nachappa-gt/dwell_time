...
def main():
    ...
    conf = SparkConf().setAppName('ScienceCoreExtension_Join'+ '/' +country + '/' + logtype)
    ...
    avro_path = os.path.join(avro_base_dir, date_path, '{fill,nf}/{tll,pos}')
    df_all = hiveContext.read.format("com.databricks.spark.avro").load(avro_path)
            
    ard_path = os.path.join(ard_base_dir, date_path) 
    ...
    avro_path_re = os.path.join(avro_base_dir, date_path,'{fill,nf}/{rest}')                            
    
    df_rest = hiveContext.read.format("com.databricks.spark.avro").load(avro_path_re)
    df_rest = df_rest.select('r_timestamp', 'request_id', 'pub_id', 'tsrc_id', 'sp_iab_category', 'user_iab_category',
    'user_ip', 'city', 'state', 'zip', 'country', 'latitude','longitude', 'sl_adjusted_confidence',
    'sl_json', 'fp_sic', 'fp_brand', 'uid', 'uid_type', 'uid_hash_type', 'age', 'gender', 'carrier',
    'os', 'device_os_version', 'device_make', 'device_model', 'device_year', 'device_type',
    'pub_type', 'bundle', 'sp_user_age', 'sp_user_gender','int_banner','isp', 'too_freq_uid',
    ...
    df_final = df.unionAll(df_rest) 
      
    output_path = os.path.join(output_base_dir, date_path)
    df_final.write.mode("overwrite").format("orc").option("compression","zlib").mode("overwrite").partitionBy('fill','loc_score').save(output_path)
...
def save_tll_pos(hiveContext,country, logtype, year, month, day, hour):
...
def save_rest_orc(hiveContext,country, logtype, year, month, day, hour, avro_partitions):
    ...
    avro_path_re = os.path.join(avro_base_dir, date_dir,'{fill,nf}/{rest}')                            
    output_path = os.path.join(output_base_dir, date_dir)

    udfAddFill = udf(add_fill, StringType())
    udfAddLocScore = udf(add_locscore, StringType())

    df_rest = hiveContext.read.format("com.databricks.spark.avro").load(avro_path_re)
    df = df_rest.select('r_timestamp', 'request_id', 'pub_id', 'tsrc_id', 'sp_iab_category', 'user_iab_category',
    'user_ip', 'city', 'state', 'zip', 'country', 'latitude','longitude', 'sl_adjusted_confidence',
    'sl_json', 'fp_sic', 'fp_brand', 'uid', 'uid_type', 'uid_hash_type', 'age', 'gender', 'carrier',
    'os', 'device_os_version', 'device_make', 'device_model', 'device_year', 'device_type',
    'pub_type', 'bundle', 'sp_user_age', 'sp_user_gender','int_banner','isp', 'too_freq_uid',
